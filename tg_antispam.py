...
blocked_text = r'http|https|t.me|join|@|پروپلیر|پرو پلیر|ولف|لفت|اتک|ننت|کص|کیر|گاییدم|کیریستال|کریستال'
					...
					if from_user.id not in exempt_users:
						if message.entities:
							...
							if re.search(blocked_type, msg_types):
								message.reply_text('**متاسفانه من از اعضای گپ کریستال کبیر بودم و شما توسط ضد لینک من بگا رفتید😔**')
									...
									for user in text_dict[text]:
										if not (len(str(user)) < 4):
											if user != from_user.id:
												try:
													app.send_message(chat_id=user, text='**متاسفانه من از اعضای گپ کریستال کبیر بودم و شما توسط ضد لینک من بگا رفتید😔**')
													app.block_user(user)
